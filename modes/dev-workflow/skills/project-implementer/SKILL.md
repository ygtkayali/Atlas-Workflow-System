...
- preserve the project's existing architecture, style, and conventions while making the approved change,
- prefer clean, reusable, modular code over task-specific patches when the touched area is part of the project's durable architecture,
- make the smallest complete change that solves the approved task without bloating code, notebooks, or surrounding files,
...
- solve architectural or reusable-code problems with temporary task-specific workarounds,
- add abstraction, indirection, or file churn that is not needed for the approved change,

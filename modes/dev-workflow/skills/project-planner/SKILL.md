...
**context-map.md**, when present, is the default project-wide entry point: it identifies authoritative files, metadata, and recommended entry rules for common task types.

**Feature subject notes** are the local context for a bounded area of work. If one exists, treat it as the default entry point for feature-local planning context — relevant design choices, task notes, decisions, recent reports, and follow-up items.

Projects may also have optional hub notes. If a hub exists alongside context-map.md, the hub supplements rather than replaces the context map. If they appear to disagree, surface the mismatch explicitly.

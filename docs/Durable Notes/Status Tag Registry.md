...
Decisions:
- v1 status tags are `[[status-draft]]`, `[[status-active]]`, `[[status-pending]]`, `[[status-settled]]`, `[[status-archived]]`
...
- `[[status-active]]`
  The note has been promoted out of draft/backlog and is now active working context, but is not yet settled.
...
- a promoted idea note usually moves to `[[status-active]]` while it is being worked through
